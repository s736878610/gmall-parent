...
public class RedisChannelConfig {
    ...
    RedisMessageListenerContainer container(RedisConnectionFactory connectionFactory,
        ...
        //订阅主题
        container.addMessageListener(listenerAdapter, new PatternTopic("seckillpush"));// redis的channel名
     ...
     */
    @Bean
    MessageListenerAdapter listenerAdapter(MessageReceive receiver) {// MessageReceive是自定义的监听类
        //这个地方 是给 messageListenerAdapter 传入一个消息接受的处理器，利用反射的方法调用“receiveMessage”
        //也有好几个重载方法，这边默认调用处理器的方法 叫handleMessage 可以自己到源码里面看
        return new MessageListenerAdapter(receiver, "receiveMessage");// receiveMessage是自定义类中的方法名
    }
